...
> 编程中的积类型数据结构，很多地方都是在以元组进行演示。猜测是把类型看作集合后，集合到集合间的态射对整个源集合元素都有意义，因此这样演示
>
> 个人理解里，并非只有元组才能看作积。例如班级信息包含年级、班、学生数量字段，这三者可以都是`Int`，但在业务意义上存在限制(例如1-6年级最多3个班最多60人)，可以看作`(1~6) * (1~3) * (1~60)`的积
